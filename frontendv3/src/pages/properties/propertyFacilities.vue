...
<script setup>
    ...
    const getData = async (page = state.listData.current_page) => {
        ...
        const response = await apiGetData('/property_facilities/index', payload);
    ...
    const save = async () => {
        ...
        const payload = {
            ...
            property_odata : state.form.property_odata,
            facility_odata: state.form.facility_odata
        };

        let response;
        if (action.value === 'add') {
            response = await apiPostData('/property_facilities/store', payload);
        }else {
            response = await apiPostData('/property_facilities/update', payload);
        }
