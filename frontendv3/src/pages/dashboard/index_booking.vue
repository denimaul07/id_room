...
<script setup>
                    ...
                    roomCalendar.forEach((cell) => {
                        ...
                        if (!isAvailableStatus(cell.status)) {
                            ...
                            const targetResourceId = targetSubRoom
                                ...
                                : (targetSubOdata ? `sub-${targetSubOdata}` : null);
    ...
    const handleEventClick = async (info) => {
        ...
        if (isAvailableStatus(status)) {
            ...
            modalAdd.value = true;
        } else if ((isBlockedStatus(status) || isPreparedStatus(status)) && booking_odata) {
            const statusLabel = isPreparedStatus(status) ? 'prepared' : 'blocked';
            Swal.fire({
                title: 'Open Room Status',
                text: `Status ${statusLabel} akan dibuka kembali jadi available.\nRoom ID: ${room_id}, Date: ${date}, User: ${booking_user || '-'}`,
                icon: 'warning',
