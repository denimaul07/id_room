<template>
    <div>
        <Button class="btn btn-primary mb-3" @click="add">Add Services</Button>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <tr v-if="state.listData.length === 0">
                        <td colspan="6" class="text-center">No Services Added</td>
                    </tr>
                    <tr v-for="(item, index) in state.listData" :key="index" v-else>
                        <td class="text-center">{{ index + 1 }}</td>
                        <td class="text-center text-nowrap">
                            <a-tooltip title="View / Edit Services">
    ...
    <a-drawer v-model:visible="modalAdd" :width="400" :closable="false" :mask-closable="true" :title="action === 'add' ? 'Add Services' : 'Edit Services'">
        <div class="mb-3 row">
            <label class="col-sm-3 col-form-label">Title</label>
            <div class="col-sm-9">
                <input type="text" class="form-control" v-model="state.form.title" placeholder="Masukan Services Title"/>
            </div>
        </div>
        ...
        <div class="mb-3 row">
            ...
            <div class="col-sm-9">
                <textarea class="form-control" v-model="state.form.desc" placeholder="Masukan Services Description"></textarea>
            </div>
        </div>
        ...
        <div class="mb-3 row">
            ...
            <div class="col-sm-9">
                <a-select
                    ...
                    style="width: 100%;"
                    show-search
                    placeholder="Pilih Icon Service"
                    size="large"
...
<script setup>
    ...
    const getData = async () => {
        loading.value = true;
        const response = await apiGetData('/setting/services', {});
    ...
    const save = async () => {
        ...
        const response = await apiPostData('/setting/services', payload);
    ...
    const update = async () => {
        ...
        const response = await apiPutData('/setting/services', payload);
    ...
    const deleteItem = async (item) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const response = await apiDeleteData('/setting/services', {
                    odata: item.odata
                });
