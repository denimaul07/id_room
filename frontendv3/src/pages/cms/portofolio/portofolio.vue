<template>
    <div>
        <Button class="btn btn-primary mb-3" @click="add">Add Portofolio</Button>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    <tr v-else-if="state.listData.length === 0">
                        <td colspan="6" class="text-center"><a-empty description="No Portofolio Added" /></td>
                    </tr>
                    <tr v-for="(item, index) in state.listData" :key="index" v-else>
                        <td class="text-center">{{ index + 1 }}</td>
                        <td class="text-center">
                            <a-tooltip title="View / Edit Portofolio">
                            ...
                            <a-tooltip title="Delete Portofolio">
    ...
    <a-drawer v-model:visible="modalAdd" :width="400" :closable="false" :mask-closable="true" :title="action === 'add' ? 'Add Portofolio' : 'Edit Portofolio'">
        <div class="mb-3 row">
            <label class="col-sm-3 col-form-label">Portofolio Name</label>
            <div class="col-sm-9">
                <input type="text" class="form-control" v-model="state.form.nama" placeholder="Masukan Portofolio Name"/>
            </div>
        </div>
...
<script setup>
    ...
    const getData = async () => {
        loading.value = true;
        const response = await apiGetData('/setting/portofolio', {});
    ...
    const save = async () => {
        ...
        const response = await apiPostData('/setting/portofolio_store', formData, {
            headers: {
        ...
        if (response) {
            ...
            Swal.fire({
                icon: 'success',
                title: 'Success',
                text: 'Portofolio has been saved successfully.',
    ...
    const update = async () => {
        ...
        const response = await apiPostData('/setting/portofolio_update', formData, {
            headers: {
                'Content-Type': 'multipart/form-data'
            }
        });
        ...
        if (response) {
            ...
            Swal.fire({
                icon: 'success',
                title: 'Success',
                text: 'Portofolio has been updated successfully.',
                confirmButtonColor: '#2c323f',
                confirmButtonText: '<i class="fa fa-check me-2"></i> OKE',
            });
    ...
    const deleteItem = async (item) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const response = await apiDeleteData('/setting/portofolio', {
                    odata: item.odata
                });
